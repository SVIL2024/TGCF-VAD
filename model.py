...
class Model(nn.Module):
    def __init__(self, args):
        ...
        if self.feature_group == 'both':
            if args.fusion == 'concat':
                ...
                self.fc1 = nn.Linear(args.feature_size + args.emb_dim, 512)
            elif args.fusion == 'crossattn_3glu':
                self.fc_vis = nn.Sequential(
                    nn.Linear(args.feature_size, 1024),
                    nn.LayerNorm(1024),
                    nn.GELU()
                )
                self.fc_text = nn.Sequential(
                    nn.Linear(args.emb_dim, 1024),
                    nn.LayerNorm(1024),
                    nn.GELU()
                )
                ...
                self.norm_ffn = nn.LayerNorm(1024)
            ...
            elif args.fusion == 'crossattn_2glu':
                self.fc_vis = nn.Sequential(
                    nn.Linear(args.feature_size, 1024),
                    nn.LayerNorm(1024),
                    nn.GELU()
    ...
    def forward(self, inputs, text):
        ...
        if self.fusion == 'concat':
            ...
            out = out + out2
        elif self.fusion == 'crossattn_3glu':
            q = self.drop_out(self.fc_vis(out))
            k = self.drop_out(self.fc_text(out2))
            v = self.drop_out(self.fc_text(out2))
            attn_out, _ = self.cross_attn(q,k,v)
            H = torch.cat([q, k, attn_out], dim=-1)
            G = self.glu(H)
            g_q, g_k, g_attn = torch.split(G, 1024, dim=-1)

            out = g_q * q + g_k * k + g_attn * attn_out
            out = self.norm_ffn(out + self.ffn(out))
        elif self.fusion == 'crossattn_2glu':
            q = self.drop_out(self.fc_vis(out))
            k = self.drop_out(self.fc_text(out2))
            v = self.drop_out(self.fc_text(out2))
            attn_out, _ = self.cross_attn(q,k,v)
            H = torch.cat([q, attn_out], dim=-1)
            G = self.glu(H)
            g_q, g_attn = torch.split(G, 1024, dim=-1)

            out = g_q * q + g_attn * attn_out
            out = self.norm_ffn(out + self.ffn(out))
        elif self.fusion == '2glu':
            out = self.drop_out(self.fc_vis(out))
            out2 = self.drop_out(self.fc_text(out2))
            H = torch.cat([out, out2], dim=-1)
            G = self.glu(H)
            g_1, g_2 = torch.split(G, 1024, dim=-1)

            out = g_1 * out + g_2 * out2
            out = self.norm_ffn(out + self.ffn(out))
        elif self.fusion == 'crossattn1':
            q = self.drop_out(self.fc_vis(out))
            k = self.drop_out(self.fc_text(out2))
            v = self.drop_out(self.fc_text(out2))
            attn_out, _ = self.cross_attn(q,k,v)
            out = self.norm_ffn(q + self.ffn(attn_out))
        else:
