...
def test(dataloader, model, args, viz, device):
    with torch.no_grad():
        ...
        for i, (input, text, scene_idx) in enumerate(dataloader):  # test set has 199 videos
            input = input.to(device)
            input = input.permute(0, 2, 1, 3)
            text = text.to(device)
            text = text.permute(0, 2, 1, 3)
            scene_idx = scene_idx.to(device)
            ...
            scores_nor_bottom, scores_nor_abn_bag, feat_magnitudes = model(input, text, scene_idx)  # 注意这里的score_abnormal和score_normal是一维的，是每一个video的一个分数，而logits则是一个T维的vector给每一个snippet都打了分
